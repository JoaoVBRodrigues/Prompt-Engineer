...
# Post 1
ID1=$(wp post create --post_type=prompt --post_title='Análise de Código com CoT' --post_status=publish --fields=ID --format=ids --allow-root | tr -d '[:space:]')
...
# Post 2
ID2=$(wp post create --post_type=prompt --post_title='Gerador de Componentes Vue' --post_status=publish --fields=ID --format=ids --allow-root | tr -d '[:space:]')
...
# Post 3
ID3=$(wp post create --post_type=prompt --post_title='Cyberpunk Cityscape' --post_status=publish --fields=ID --format=ids --allow-root | tr -d '[:space:]')
